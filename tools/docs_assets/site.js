(() => {
  ...
  const items = Array.isArray(globalThis.ChronoKitSearchIndex) ? globalThis.ChronoKitSearchIndex : [];
  const colorPreference = window.matchMedia("(prefers-color-scheme: dark)");

  function storedTheme() {
    try {
      const value = localStorage.getItem("chronokit-theme");
  ...
  themeToggle?.addEventListener("click", () => {
    ...
    try { localStorage.setItem("chronokit-theme", next); } catch (_error) { /* Session-only theme is still useful. */ }
    updateThemeControl();
  });
