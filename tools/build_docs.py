#!/usr/bin/env python3
"""Build ChronoKit-FP Markdown documentation as a small, dependency-free site."""
...
OUTPUT_MARKER = ".chronokit-fp-docs-output"
...
def load_layout(source: Path, config: SiteConfig) -> DocumentationLayout:
    ...
    try:
        ...
        if schema == 1:
            ...
            return DocumentationLayout("ChronoKit-FP documentation", "Practical ChronoKit-FP documentation for Free Pascal and Lazarus.", legacy_navigation(source), tuple(), {}, legacy=True)
        if schema != 2:
...
<title>{html.escape(title)} — ChronoKit-FP</title><script>try{{var t=localStorage.getItem('chronokit-theme');if(t==='light'||t==='dark')document.documentElement.dataset.theme=t}}catch(e){{}}</script><link rel="stylesheet" href="{html.escape(stylesheet, quote=True)}"></head>
...
def write_landing_page(site_root: Path, config: SiteConfig) -> None:
    ...
    (site_root / "index.html").write_text(f"""<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta http-equiv="refresh" content="0; url={target}"><title>ChronoKit-FP documentation</title></head><body><main><p>Opening <a href="{target}">ChronoKit-FP {html.escape(config.current)} documentation</a>.</p></main></body></html>\n""", encoding="utf-8")
...
def write_offline_archive(site_root: Path, archive: Path, release: str) -> str:
    archive.parent.mkdir(parents=True, exist_ok=True); root_name = f"chronokit-fp-docs-{release}"
...
def build_site(source: Path, output: Path, site_root: Path, versions_path: Path, offline_archive: Path | None = None, release: str | None = None) -> int:
    ...
    validate_source_links(source, documents, source.parent); prepare_output(output, config.release); copy_assets(output); copy_document_assets(source, output)
    ...
    print(f"Built {len(documents)} ChronoKit-FP documentation pages in {output}")
