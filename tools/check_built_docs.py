...
def check_page(page: Path, site: Path, release: str, current: str, releases: set[str], version_dir: Path) -> list[str]:
    errors: list[str] = []
    ...
    for nav_link in parsed.nav_links:
        ...
        try:
            ...
            continue
        if any(pattern.match(relative.as_posix()) for pattern in NAV_INTERNAL_PATTERNS):
...
def check_site(site: Path) -> list[str]:
    ...
    for entry in entries:
        ...
        for page in pages:
            errors.extend(check_page(page, site, release, current, release_names, directory))
    return errors
