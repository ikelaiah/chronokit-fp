...
| How do I find a period boundary or round a value? | `StartOf*`, `EndOf*`, `FloorDate`, `CeilingDate`, `RoundDate` | Documentation defects. The cheat sheet's additional rounding example uses nonexistent `drsDay`, `drsWeek`, `drsMonth`, and `drsYear` values instead of `du*`; it omits the hour methods; and it does not disclose that the public but unimplemented `duSeason` value currently leaves input unchanged. | Correct the enum values, cover all boundary methods, and document the season limitation without expanding this discovery milestone into a rounding behavior change. |
